...
# 15. Create master or aggregated data frames of NDVI values 
...
# 20. Create data frame of outliers    
...
generate_boxplot <- function(x_variable, y_variable, title_text, subtitle_text, label_outliers){
  ggplot2::ggplot(df_ndvi_boxplot, aes_string(x = x_variable, y = y_variable, fill = x_variable)) + 
    ...
    if(label_outliers == TRUE){
      list(geom_point(data = df_outliers, color = 'red', show.legend = FALSE), ggrepel::geom_text_repel(data = df_outliers, aes(label = outlier), force = 0.5, size = 2.25, hjust = 0, na.rm = TRUE, direction = 'y'))
      } 
}
