...
Parameters:
  ...
  report_author: author name for report header
  report dat: date for report header
  ...
  list_shp_ground: Give paths to ground truth shapefiles
  crop_type: name of attribute table field within each ground truth shapefile that identifies crop type
  list_truthing_dates: Give string list with MM/DD/YY format of imagery dates that most closely match ground truth dates
  epsg: EPSG code to which data will be re-projected
...
knitr::opts_chunk$set(fig_caption = TRUE, message = FALSE)
...
# 3. Create dataframe lookup table of crop number (key) to name
...
# 4. Read in both Program Fallow and Ground Truth shapefile feature classes and generate a reprojecetd simple feature access for each
...
sf_program_fallow <- params$shp_program %>% sf::st_read() %>% sf::st_transform(crs = params$epsg) %>% dplyr::rename_all(toupper) %>% dplyr::rename(FIELD_ID = !!as.name(params$identifier))
...
# Create function to create simple features data frame, reproject it, set all column names to upper case, and rename user selected column to CROP_TYPE
read_and_reproject <- function(X){
  X %>% sf::st_read() %>% sf::st_transform(crs = params$epsg) %>% dplyr::rename_all(toupper) %>% dplyr::rename(CROP_TYPE = !!as.name(params$crop_type))
}
...
# 6. Generate tables of sum acreage by crop type for each time period
...
summarize_crop_acreage <- function(i){
  ...
  sfs_ground_data[[i]] %>% as.data.frame() %>% dplyr::mutate('Crop' = df_crop_key$crop_name[match(CROP_TYPE, df_crop_key$crop_code)])%>% dplyr::group_by(Crop) %>% summarise(Acres = round(sum(ACRES), 2), Frequency = n()) %>% knitr::kable(caption = caption_text, row.names = FALSE)   
} 
...
# 8. Create two lists of cropped bricks: one list cropped by maximum extent of all feature classes, the second cropped by Program Fallow Fields
...
# 10. Make false color map of each Sentinel image overlaid with Program Fallow Fields

# OUTPUTS: sf_program_southern, northern

#--------------------------------------------------------------------------------

# Crop Program Fallow Fields to split in two
sf_program_southern <- st_crop(sf_program_fallow, xmin = extent_program[1], xmax = extent_program[2], ymin = extent_program[3], ymax = abs(extent_program[4] - extent_program[3])/2 + extent_program[3]) 
sf_program_northern <- st_crop(sf_program_fallow, xmin = extent_program[1], xmax = extent_program[2], ymin = abs(extent_program[4] - extent_program[3])/2 + extent_program[3], ymax = extent_program[4])
...
# 10a. Make false color map of each Sentinel image overlaid with Program Fallow Fields

# OUTPUTS: cir_northern_southern

#--------------------------------------------------------------------------------

# Define function to plot split color-infrared images
plot_cir_split <- function(i, sf_program_cropped){
  layer_name <- paste0('Color-Infrared Image ', names(bricks_cropped_program)[[i]])
  ggplot(data = sf_program_cropped) + 
  RStoolbox::ggRGB(img = bricks_cropped_program[[i]], r = 4, g = 3, b = 2, stretch = 'hist', ggLayer = T) + xlim(extent(sf_program_cropped)[1:2]) + ylim(extent(sf_program_cropped)[3:4]) + coord_equal() +
  ...
  guides(col = guide_legend(ncol = 9, title = 'Field ID Key')) +  
  ggtitle(layer_name) + 
  theme(plot.title = element_text(hjust = 0.5),legend.text = element_text(margin = margin(0, 0, 0, -24)), legend.key.width = unit(20, 'points'), legend.key.size = unit(0.5, 'line'), legend.position = 'bottom') +
  labs(x = 'Longitude', y = 'Latitude') +
  ggspatial::annotation_scale(style = 'ticks') + 
  ggspatial::annotation_north_arrow(location = 'br', style = north_arrow_minimal)    
}

# Apply cropped bricks to function to plot color-infrared northern fields; assign names
cir_maps_northern <- lapply(X = seq_along(bricks_cropped_program), sf_program_cropped = sf_program_northern, FUN = plot_cir_split)
names(cir_maps_northern) <- paste('Color-infrared image', names_bricks, 'overlaid with northern program fallow fields')

# Apply cropped bricks to function to plot color-infrared southern fields; assign names
cir_maps_southern <- lapply(X = seq_along(bricks_cropped_program), sf_program_cropped = sf_program_southern, FUN = plot_cir_split)
names(cir_maps_southern) <- paste('Color-infrared image', names_bricks, 'overlaid with southern program fallow fields')

# Combine northern and southern maps
cir_northern_southern <- c(cir_maps_northern, cir_maps_southern)

# Order combined maps by date
cir_northern_southern <- cir_northern_southern[order(names(cir_northern_southern))]
...
# 10b. Make false color map of each Sentinel image overlaid with Program Fallow Fields
...
plot_cir <- function(i){
  ...
  ggtitle(layer_name) + 
  theme(plot.title = element_text(hjust = 0.5),legend.text = element_text(margin = margin(0, 0, 0, -24)), legend.key.width = unit(20, 'points')) + labs(x = 'Longitude', y = 'Latitude') +
  ggspatial::annotation_scale(style = 'ticks') + 
  ggspatial::annotation_north_arrow(location = 'br', style = north_arrow_minimal)    
}
...
names(cir_maps) <- paste('Color-infrared image', names_bricks, 'overlaid with program fallow fields')

```


```{r, eval = TRUE, include = FALSE, echo = FALSE, warning = FALSE, message = FALSE, results = FALSE}

# 11. Make NDVI maps from Sentinel image overlaid with fallow fields
...
plot_ndvi_split <- function(i, sf_program_cropped){
  
  ggplot() + 
    geom_raster(data = rasters_ndvi_program_extent[[i]], aes(x = x, y = y, fill = layer)) +
    coord_equal() +
    scale_y_continuous(limits = c(extent(sf_program_cropped)[3], extent(sf_program_cropped)[4])) +
    coord_cartesian(expand = TRUE) +
    geom_sf(data = sf_program_cropped, aes(color = FIELD_KEY), size = 1.15, alpha = 0) +
    geom_sf_text(data = sf_program_cropped, aes(label = ID), fontface = 'bold', color = 'white', size = 2.5) +
    scale_fill_viridis_c(name = 'NDVI', option = 'plasma', direction = -1, limits = c(-1, 1)) +
    scale_color_discrete(breaks = gtools::mixedsort(sf_program_cropped$FIELD_KEY)) +
    guides(colour = FALSE) +
...
# Apply cropped bricks to function to plot color-infrared northern fields; assign names
ndvi_maps_northern <- lapply(X = seq_along(bricks_cropped_program), sf_program_cropped = sf_program_northern, FUN = plot_ndvi_split)
names(ndvi_maps_northern) <- paste('NDVI image', names_bricks, 'overlaid with northern program fallow fields')

# Apply cropped bricks to function to plot color-infrared southern fields; assign names
ndvi_maps_southern <- lapply(X = seq_along(bricks_cropped_program), sf_program_cropped = sf_program_southern, FUN = plot_ndvi_split)
names(ndvi_maps_southern) <- paste('NDVI image', names_bricks, 'overlaid with southern program fallow fields')

# Combine northern and southern maps
ndvi_northern_southern <- c(ndvi_maps_northern, ndvi_maps_southern)

# Order combined maps by date
ndvi_northern_southern <- ndvi_northern_southern[order(names(ndvi_northern_southern))]
...
names(ndvi_maps) <- paste('NDVI image', names_bricks, 'overlaid with program fallow fields')
...
# 12. Calculate zonal mean by Program Fallow Fields for each NDVI raster date
...
# 13. Zonal mean by Ground Truth Fields for each NDVI raster date
...
# 14. Create master or aggregated data frames of NDVI values 

# OUTPUTS: df_zonal_ndvi_program, df_master_zonal_ndvi, df_master_average_ndvi, df_ndvi_boxplot

#--------------------------------------------------------------------------------

# Generate dataframe of all NDVI values for each Program Fallow Field for each date
df_zonal_ndvi_program <- bind_rows(dfs_zonal_ndvi_program)

#--------------------------------------------------------------------------------

# Generate master dataframe of all NDVI values for each field (either Program Fallow or Ground Truth) for each date
...
df_master_zonal_ndvi <- df_master_zonal_ndvi  %>% mutate_at('Date', factor) %>% dplyr::mutate(crop_vs_fallow = ifelse(!(FIELD_TYPE %in% c('Fallow', 'Program Fallow')), 'Crop', ifelse(FIELD_TYPE == 'Fallow', 'Fallow', ifelse(FIELD_TYPE == 'Program Fallow', 'Program Fallow', ''))))

#--------------------------------------------------------------------------------

# Create master data frame of mean NDVI aggregated by date
...
# Create dataframe of all NDVI values to include in boxplot (same as df_master_zonal_ndvi except eliminating those for Ground Truth Fields known to be fallow for dates not corresponding to ground truth dates)  
df_ndvi_boxplot <- df_master_zonal_ndvi %>% dplyr::mutate(boxplot_ndvi = ifelse(crop_vs_fallow == 'Fallow' & !(Date %in% params$list_truthing_dates), as.numeric(NA), NDVI)) %>% dplyr::filter( boxplot_ndvi, !is.na(boxplot_ndvi))
...
# 15. Create compiled dataframe of all field-scale NDVI values (Program Fallow Fields and each crop type) and check normality
...
# 16. Create line graph showing NDVI of each individual Program Fallow Field
...
# 17. Create list of line graphs for each Program Fallow Field with its respective line highlighted
...
# Give names to list of highlighted NDVI graphs
names(lines_zonal_ndvi_highlighted) <- paste('NDVI time series highlighting farm field ID', fields)
...
# 19. Create data frame of outliers    
 
# OUTPUTS: df_outliers, df_ndvi_boxplot
...
# 20. Generate boxplots of NDVI by crop type (ungrouped and grouped)
...
generate_boxplot <- function(x_variable, y_variable, title_text, subtitle_text, label_outliers){
  ggplot2::ggplot(df_ndvi_boxplot, aes_string(x = x_variable, y = y_variable, fill = x_variable)) + 
    ...
    if(label_outliers == TRUE){
      list(geom_point(data = df_outliers, color = 'red', show.legend = FALSE), ggrepel::geom_text_repel(data = df_outliers, aes(label = outlier), force = 1.25, segment.size = 0.2, segment.alpha = 0, size = 2, hjust = -1, direction = 'y', na.rm = TRUE, point.padding =  0.5))
      } 
}
...
# 21. Create table NDVI values for Program Fallow fields
...
df_ndvi_program_wider <- df_zonal_ndvi_program %>% dplyr::select('Date', 'FIELD_ID', 'NDVI') %>% dplyr::mutate_if(is.numeric, round, 2) %>% tidyr::spread(key = Date, value = NDVI) %>% dplyr::arrange(as.numeric(FIELD_ID))
...
```{r, eval = TRUE, include = TRUE, warnings = FALSE, message = FALSE, echo = FALSE, fig.width = 8, fig.height = 11, fig.cap = names(cir_northern_southern)}

for(c in cir_northern_southern){
  print(c)
  cat('\n\n') 
}
...
```{r, eval = TRUE, include = TRUE, warnings = FALSE, message = FALSE, echo = FALSE, fig.width = 8, fig.height = 11, fig.cap = names(ndvi_northern_southern)}

for(m in ndvi_northern_southern){
