...
calculate_ndvi_zonal_program <- function(i, simple_feature, field_type){
  ...
  ndvi_fallowed_fields <- raster::extract(x = rasters_ndvi_max_extent[[i]], 
                           y = sf_program_fallow,
                           method = 'simple',
                           fun = mean,
                           na.rm = T,
...
# Therfore, we use non-parametic test for unpaired (independent), continuous data (Wilcoxon Rank-Sum test, a.k.a. Mann-Whitney U-test, a.k.a Wilcoxon-Matt-Whitney test).
...
# 20. Make box plot showing months side by side

# OUTPUTS: boxplotCropTypeNDVI, boxplotCroppedorFallowNDVI

#--------------------------------------------------------------------------------

# Reorder observations by month so that they appear chronologically in box plot

# month_vector <- unlist(month_list)
# df_master_zonal_ndvi$monthFactors <- factor(df_master_zonal_ndvi$Month, levels = month_vector)


# Use mutate() and nested ifelse() to create new variable with the following three factors: 1) cropped fields (Ground Truth Fields excluding known fallow), 2) Ground Truth Fields known to be fallow, 3) Program Fallow Fields
...
# Create function to make boxplots with facets by month
CreateBoxplots <- function(boxplotCategory, y, titleText, subtitleText){
  # Graph boxplot of NDVI by crop type by month
  boxplotCropTypeNDVI <- ggplot2::ggplot(df_ndvi_boxplot, aes_string(x = boxplotCategory, y = y, fill = boxplotCategory)) + # NOTE: aes_string required instead of aes when passing parameter from user defined function
    ...
    theme(plot.title = element_text(hjust = 0.5, size = 22),
          ...
          axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0.25, size = 7),
          strip.text.x = element_text(size = 8, colour = 'black')) + 
    labs(x = 'Field Type', subtitle = paste0(subtitleText, ' \n')) + 
    facet_grid(.~ Date) + # Breaks boxplots into frame for each month
    guides(fill = guide_legend(title = 'Field Type',
                               title.position = 'top',
                               title.hjust = 0.5))
}

# Call function to make two box plots 

# Boxplot with categories where each crop type is independent
boxplotCropTypeNDVI <- CreateBoxplots('FIELD_TYPE', 'NDVI', 'Mean NDVI Values by Field Type', params$area_name)

#boxplotCropTypeNDVI


CreateBoxplotsGroupedCropped <- function(boxplotCategory, y, titleText, subtitleText){
  # Graph boxplot of NDVI by crop type by month
  boxplotCropTypeNDVI <- ggplot2::ggplot(df_ndvi_boxplot, aes_string(x = boxplotCategory, y = y, fill = boxplotCategory)) + # NOTE: aes_string required instead of aes when passing parameter from user defined function
    #geom_boxplot(alpha = 0.7, outlier.colour = ifelse(is.na(df_master_zonal_ndvi$outlier), 'grey50', 'red')) + # Monkeys. Why doesn't this work
    geom_boxplot(alpha = 0.7) +
    geom_point(data = df_outliers, color = 'red', show.legend = FALSE) +
    ggtitle(titleText) +
    theme(plot.title = element_text(hjust = 0.5, size = 22),
          plot.subtitle = element_text(hjust = 0.5, size = 18),
          legend.position = 'bottom',
    ...
    guides(fill = guide_legend(title = 'Field Type',
                               title.position = 'top',
                               title.hjust = 0.5)) +
          ggrepel::geom_text_repel(aes(label = outlier), direction = 'y', size = 2, hjust = 0, na.rm = TRUE, force = 0.5, nudge_x = 2) 
}

# Boxplot with any crop field concatenated 
boxplotCroppedorFallowNDVI <- CreateBoxplotsGroupedCropped(boxplotCategory = 'crop_vs_fallow', y = 'NDVI', titleText = 'NDVI of Cropped vs Ground Truth Fallow vs Fallow Program Fields', subtitleText = params$area_name)

#boxplotCroppedorFallowNDVI
```
